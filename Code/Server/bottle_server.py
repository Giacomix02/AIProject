...
def predict_post():
    ...
    lower_limit = request.query["lower_limit"]  # ricevo il limite minore per il filtro (opzionale)
    upper_limit = request.query["upper_limit"]  # ricevo il limite superiore per il filtro (opzionale)
    ...
    if lower_limit is not None or upper_limit is not None:  # se uno o più dei limiti sono specificati
        ...
        while len(variables) != 0:  # fino a che ci sono variabili
            ...
            if results[var_x] == 1:  # se il numero x è 1, non va scartato
                to_return[str(var_x)] = results[var_v]  # lo aggiungo al dizionario da ritornare
...
def predict_post():
    ...
    while len(variables) != 0:
        ...
        var_x = variables.pop()
        if results[var_x] == 1:
            to_return[str(var_x)] = results[var_v]
